...
try {
  ...
  console.error('Failed to ensure data directory or warns file exists:', err);
  process.exit(1);
}
...
function loadWarns() {
  try {
    ...
    try { fs.writeFileSync(WARNS_FILE, JSON.stringify({}), 'utf8'); } catch (e) { console.error('Failed to reset warns.json:', e); }
    return {};
  }
}
function saveWarns(data) {
  try {
    ...
    console.error('Failed to save warns.json:', err);
  }
}
      ...
      for (const channel of guild.channels.cache.values()) {
        try {
          if (channel && typeof channel.isTextBased === 'function' && channel.isTextBased()) {
            // Use boolean permission overwrites — discord.js will accept the camelCase keys here
  ...
  try {
    ...
    switch (cmd) {
      ...
      case 'tempban': {
        ...
        setTimeout(async () => {
          try {
            // Use guild.members.unban for clarity
            await message.guild.members.unban(member.id).catch(() => {});
            message.channel.send(`${member.user.tag} was automatically unbanned (tempban expired).`).catch(() => {});
          } catch (err) {
            // ignore
          }
        }, ms);
      ...
      case 'warn': {
        ...
        warns[message.guild.id][user.id].push({ moderator: message.author.id, reason, timestamp: Date.now() });
        saveWarns(warns);
    ...
    try { message.reply('An error occurred while running that command.'); } catch (e) { console.error('Failed to send error reply:', e); }
  }
});

// Simple duration parser: 1d 2h 30m (supports optional spaces)
